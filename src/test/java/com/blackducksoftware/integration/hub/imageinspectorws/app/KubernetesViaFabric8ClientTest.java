...
public class KubernetesViaFabric8ClientTest {
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        final List<HasMetadata> resultPod = client.load(KubernetesViaFabric8ClientTest.class.getResourceAsStream("/kube-pod.yml")).inNamespace("default").createOrReplace();
        final List<HasMetadata> resultSvc = client.load(KubernetesViaFabric8ClientTest.class.getResourceAsStream("/kube-service.yml")).inNamespace("default").createOrReplace();
        ...
        Thread.sleep(20000L);
        final int healthCheckLimit = 10;
        boolean serviceIsHealthy = false;
        for (int i = 0; i < healthCheckLimit; i++) {
            Thread.sleep(10000L);
            String[] healthCheckOutput;
            try {
                System.out.printf("Health check attempt %d of %d:", i, healthCheckLimit);
                ...
                System.out.printf("Health check failed: %s", e.getMessage());
            }
        }
